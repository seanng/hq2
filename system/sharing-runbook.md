...
2. **Deliver the shared gws OAuth client out-of-band, then place it before
   running setup.** The shared Google Workspace (gws) OAuth client file is
   **git-ignored on purpose**, so it is **not** in the repo the friend just cloned.
   Deliver it to the friend over a **secure out-of-band channel** (e.g. a password
   manager share or encrypted message — not committed to the repo, not pasted in
   plaintext). Have the friend save it at `.agents/auth/gws-client-secret.json`
   inside their `~/hq2` clone **before** running `setup.sh`. If this file is
   missing, setup's gws step degrades to a warning and falls back to manual auth,
   leaving the friend stranded on Google Workspace tooling — so do this first.
3. **Run `setup.sh`.** From the repo root: `./setup.sh`. It installs prerequisites,
...
4. **First Pam session runs the IDENTITY interview.** The friend's first session
